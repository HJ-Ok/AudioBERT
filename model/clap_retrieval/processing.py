...
from transformers import (AutoProcessor, BertForTokenClassification,
                          BertTokenizer)
...
def parse_args():
    ...
    parser.add_argument(
        "--data", type=str, default="combined", required=False
    )  # animal_sounds, height_of_sounds, combined
...
def processing(args):
    ...
    if args.mode == "text":
        ...
        model = BertForTokenClassification.from_pretrained(
            "../detection_bert/detection_bert_model_combined", num_labels=2
        ).to(device)
        ...
        for idx in tqdm(range(len(df)), total=len(df)):
            ...
            if idx % 2 == 0:
                try:
                    ...
                    text_inputs = processor(
                        text=f" sound", return_tensors="pt", padding="max_length", max_length=max_len
                    )
            else:
                try:
                    text_inputs = processor(
            ...
            dic[idx] = text_inputs
        for idx in dic:
            dic[idx] = {"text_inputs": {key: value.tolist() for key, value in dic[idx].items()}}
        ...
        for idx in tqdm(range(len(df)), total=len(df)):
            ...
            max_len = 64
            inputs = processor(
                audios=[audio], sampling_rate=sr, return_tensors="pt", padding="max_length", max_length=max_len
            )
        ...
        for idx in dic:
            dic[idx] = {
                "inputs": {key: value.tolist() for key, value in dic[idx][0].items()},
